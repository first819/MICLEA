#!/usr/bin/env python3
# General interactive-halftone builder. Point SRC at any finished blue-on-white
# halftone render; it recovers the dot grid from real ink coverage and writes
# three interactive variants (round dots / even squares / uneven blocks with a
# reacting white edge). Self-contained HTML — re-run any time to regenerate.
from PIL import Image
import base64, math

# ---- config ----------------------------------------------------------------
SRC    = "art/3d/temple-blue-dashes.png"
PREFIX = "temple"
TITLE  = "Temple"

CELL    = 9
DOT_MAX = 4.6
INK     = "rgb(160,154,255)"     # render colour (light blue)
...
# ink-luminance reference = 1st percentile luminance of the source (its solid ink)
lums = sorted(L.getdata())
INK_LUM = lums[max(0, len(lums)//100)]
DENOM   = max(1.0, 255.0 - INK_LUM)
print("source", SRC, W, "x", H, "| ink_lum", INK_LUM)
